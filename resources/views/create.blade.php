...
@section('content') 
    <div class="max-w-2xl mx-auto bg-white shadow-md rounded-md p-6">
        <h1 class="text-2xl font-bold text-blue-700 mb-6">Add New Task</h1>
        ...
        <form method="POST" action="{{ route('tasks.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5"
                          class="w-full mt-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="long_description" id="long_description" rows="10"
                          class="w-full mt-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('long_description') }}</textarea>
